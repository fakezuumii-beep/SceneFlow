...
async function reload(){const p=await api('/projects/'+pid());state.project=p;if(!p.shots.some(s=>s.id===state.selected))state.selected=p.shots[0]?.id;renderAll()}
function renderAll(){
  ...
  $('#ratio').value=p.options.broll_ratio;$('#ratioValue').textContent=p.options.broll_ratio+'%';$('#shotLength').value=p.options.max_shot;
  $('#materialSource').value=p.options.source;$('#resolution').value=p.options.resolution;$('#subtitlesToggle').checked=p.options.subtitles;
...
function renderStatus(){
  ...
  $('#arollButton').disabled=state.busy||!state.arollSupported||!p.shots.some(s=>s.kind==='A');
...
async function startJob(action,shotId=null){
  ...
  await guarded(async()=>{await api(`/projects/${pid()}/jobs`,{method:'POST',body:JSON.stringify({action,shot_id:shotId})});await reload();if(action==='render'||action==='all')setPanel('exports')});
}
...
$('#installModels').onclick=()=>guarded(async()=>{$('#settingsDialog').close();await startJob('setup_models')});
