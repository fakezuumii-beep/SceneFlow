...
- DeepSeek API：理解原文并编排 A-roll / B-roll 镜头。
...
2. 生成连续配音和逐句真实时间；Azure 失败后重试时会复用已完成段落。
3. DeepSeek 按语义与真实时间切镜头。
4. Pexels 自动匹配 B-roll。
5. 独立 MuseTalk 为 A-roll 生成人物口型。
6. FFmpeg 合成 16:9 MP4、字幕和导出清单。

**音频一键成片**

导入 MP3/WAV/M4A 后，本地 faster-whisper 先生成短语级时间，再执行同一套分镜、素材、口型和导出流程。也可以直接导入 SRT。
...
Azure TTS V1 是新项目默认引擎，默认中文音色为 `zh-CN-XiaoxiaoNeural`。界面支持中文/英文音色与 0.85×–1.2× 常用语速。它按自然句子分段请求，每段最多重试三次，并只在音频可解码后写入缓存；网络中断后再次继续会复用已经完成的段落。
